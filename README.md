...
ターミナルに以下のようなログが出力されていれば、Python実行環境のインストールに成功しています。
```sh
[2024-08-18 23:00:38] python.sh: Successfully installed Python!
[2024-08-18 23:00:38] python.sh: Detected Python 3.12.5
```
...
本スクリプトを実行すると、バージョン管理ツール、開発・実行環境、パッケージ管理ツールの動作のため、必要に応じて以下のパッケージが自動インストールされます。
